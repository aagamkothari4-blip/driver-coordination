...
**Version:** 1.1  
**Date:** May 2026  
**Changelog:** Added background location tracking, expanded location tracking scope to full job lifecycle, real-time driver status updates on manager dashboard  
...
- **Active Drivers Map**: Live view of all online drivers — map and driver list must update in real-time via WebSocket without requiring a page reload
- **Driver List**:
...
- **Job Accepted**:
  ...
  - **Real-time location sharing begins immediately on job acceptance** (not only when in transit) so the manager can track the driver en route to the pickup point
  
- **In Transit** (after car is picked up):
  - Real-time location sharing continues to backend
  - Navigation to drop-off location
  - Mark as "Delivered" button (at drop-off location)

- **Background Location Tracking**:
  - Location updates must continue even when the driver minimizes the app or the device screen locks
  - Must use a background location service (e.g. Android Foreground Service / iOS Background Modes with `location` capability in React Native, or equivalent in Flutter)
  - A persistent status bar notification must be shown on the driver's device while background tracking is active, informing the driver that location is being shared
  - Background tracking is active for the full duration of an accepted job: from acceptance → en route to pickup → in transit → delivery
  - Background tracking stops automatically when the job is marked as completed or cancelled
...
- **Location tracking is active for the entire accepted job lifecycle**: starting from job acceptance (driver en route to pickup), through the pickup, and continuing until the job is marked completed
- Driver app sends location updates every 10 seconds during an active job — this must continue even when the app is in the background or the screen is locked (via background service)
...
- Manager dashboard receives live location updates via WebSocket for both the "en route to pickup" and "in transit" phases
- **If the driver's app goes to background and location updates stop arriving**, the backend should detect the gap (no update for >30 seconds during an active job) and send a push notification to the driver prompting them to reopen the app or re-enable location permissions
